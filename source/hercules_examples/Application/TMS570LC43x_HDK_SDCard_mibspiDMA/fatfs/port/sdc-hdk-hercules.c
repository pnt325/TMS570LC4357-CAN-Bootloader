...
static int DMA_Rx_StatusFlag;
g_dmaCTRL g_dmaCTRLPKT_TX, g_dmaCTRLPKT_RX;
static uint8 spiTxBuff[128] = {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF, \
...
#define MIBSPI_DMA_MODE
#define SD_mibspiREG            mibspiREG2   //SPI2 is used on TI HDK for SD card
#define SD_mibspiRAM            mibspiRAM2

#define TG1_LENGTH   127           //MibSPI transfer group #1 length which is used to read file
                                  //MibSPI transfer group 0 is used for simple SD commands

// asserts the CS pin to the card
static
void DESELECT (void)
{
    SD_mibspiREG->PC3 |=  0x01;    // CS[0] = high
}
...
void SELECT (void)
{
    SD_mibspiREG->PC3 &= ~0x01;    // CS[0] = low
}
...
{
    uint16 TG0_RX_DATA[1]= {0x0000};
    uint16 TG0_TX_DATA[1] = {0x00FF & dat};

    mibspiSetData(SD_mibspiREG, 0, &TG0_TX_DATA[0]);
	mibspiTransfer(SD_mibspiREG, 0);

	while(mibspiIsTransferComplete(SD_mibspiREG, 0) == FALSE);
    mibspiGetData(SD_mibspiREG,0,&TG0_RX_DATA[0]);
}
...
BYTE rcvr_spi (void)
{
    ...
    return ((unsigned char)(TG0_RX_DATA[0] & 0x00FF));
}
...
static
void rcvr_spi_dma (BYTE *dst, unsigned int spiPktLen)
{
    BYTE tgPSTART1 = (SD_mibspiREG->TGCTRL[1] >> 8) & 0xFF; //Group 1
    SD_mibspiREG->TGCTRL[2] &= ~(0xFF << 8);
    SD_mibspiREG->TGCTRL[2] |=  (tgPSTART1 + spiPktLen) << 8;

    uint16 bufid;

    DMA_Rx_StatusFlag = 0;

    //addr+1 is for byte instead of uint16
    dmaConfigCtrlTxPacket((unsigned int)&spiTxBuff[0], (uint32)&(SD_mibspiRAM->tx[tgPSTART1].data)+1, spiPktLen, 1);
    dmaSetCtrlPacket(DMA_CH1, g_dmaCTRLPKT_TX);
    dmaSetChEnable(DMA_CH1, DMA_HW);    //mibSPI2 TX, hardware triggering
    dmaReqAssign(DMA_CH1, DMA_REQ3);          //mibSPI2 TX

    dmaConfigCtrlRxPacket((uint32)&(SD_mibspiRAM->rx[tgPSTART1].data)+1, (unsigned int)dst, spiPktLen, 1);
    dmaSetCtrlPacket(DMA_CH0, g_dmaCTRLPKT_RX);
    dmaSetChEnable(DMA_CH0, DMA_HW);    //MibSPI2 RX, hardware triggering
    dmaReqAssign(DMA_CH0, DMA_REQ2);           //MibSPI2 RX

    bufid  = tgPSTART1 + spiPktLen - 1;

    /* enabling transmit and receive dma */
    SD_mibspiREG->DMACTRL[1] = 0x8000C000;

    /* setting transmit (bit 16) and receive (bit 20)  channels */
    SD_mibspiREG->DMACTRL[1] |= (1 << 20) | (0 << 16);

    /* Enable ONESHOT */
    SD_mibspiREG->TGCTRL[1] |= 0x40000000; // Enable ONESHOT

    /* setting Initial Count of DMA transfers and the buffer utilized for DMA transfer */
    SD_mibspiREG->DMACTRL[1] |=  (bufid<<24);

    /* Enable Large count transfer */
    SD_mibspiREG->DMACNTLEN = 0x1;
    SD_mibspiREG->DMACOUNT[1] = (1 - 1) << 16;

    mibspiTransfer(SD_mibspiREG, 1);

    while(DMA_Rx_StatusFlag == 0);
}
...
/*-----------------------------------------------------------------------*/
/* Send 80 or so clock transitions with CS and SIMO held high. This is     */
...
void send_initial_clock_train(void)
{
    ...
    for(i = 0 ; i < 10 ; i++)
    {
        /* Write DUMMY data */
        /* FIFO. */
        xmit_spi(0xFF); //QJ changed on Sept 2020
    }
}
...
{
    /*
     * This doesn't really turn the power on
     */
...
void set_max_speed(void)
{
    ...
    /* You can change the value based on the VCLK used in your device configuration */
	SD_mibspiREG->FMT0 &= 0xFFFF00FF;   // mask out baudrate prescaler, Used by MibSPI2 TG0
	                                // Max. 5 MBit used for Data Transfer.
    SD_mibspiREG->FMT0 |= 2 << 8;     //Vclk=75MHz, baudrate prescale 75MHz / (74+1) = 1MBit

    SD_mibspiREG->FMT1 &= 0xFFFF00FF;   // mask out baudrate prescaler, used MibSPI TG1
                                     // Max. 5 MBit used for Data Transfer.
    SD_mibspiREG->FMT1 |= 2 << 8;     //VCLK=75MHz, baudrate prescale 75MHz / (74+1) = 1MBit
}
...
)
{
    ...
    if(token != 0xFE) return FALSE;    /* If not valid data token, retutn with error */ /*0xFE, QJ*/
...
{
    BYTE token;
    BYTE length = TG1_LENGTH;    //MibSPI Transfer Group1, # of buffers
    ...
    do {                            /* Receive the data block into buffer */
        if (btr < TG1_LENGTH) length=btr;  //added Sept 26, 2020
        rcvr_spi_dma(buff, length);
        buff += length;
    } while (btr -= length);
    rcvr_spi();                        /* Discard CRC */
    rcvr_spi();
...
)
{
    ...
    return res;            /* Return with the response value *//* res=0, QJ*/
}
...
)
{
    ...
    CardType = ty;
    DESELECT();            /* CS = H */
    rcvr_spi();            /* Idle (Release DO) */  //QJ comment out
...
)
{
    ...
    DESELECT();            /* CS = H */
   rcvr_spi();            /* Idle (Release DO) */ //QJ commented out
...
//            break;
        ...
        DESELECT();            /* CS = H */
        rcvr_spi();            /* Idle (Release DO) */ //QJ commented
    }
...
DWORD get_fattime (void)
{
    ...
    return    ((2020UL-2000) << 25)    // Year = 2007
            | (9UL << 21)            // Month = June
            | (26UL << 16)            // Day = 5
            | (10U << 11)            // Hour = 11
            | (38U << 5)            // Min = 38
...
/* USER CODE BEGIN (54) */
//    if (channel == DMA_CH0)  //SPI rx completed
//        DMA_Rx_StatusFlag = 1;
    if (channel == DMA_CH0)  //SPI rx completed
    {
        //Disale DMA REQ
       //SD_mibspiREG->DMACTRL[1] &=  ~(0xC << 14);
        DMA_Rx_StatusFlag = 1;
    }
/* USER CODE END */
}
