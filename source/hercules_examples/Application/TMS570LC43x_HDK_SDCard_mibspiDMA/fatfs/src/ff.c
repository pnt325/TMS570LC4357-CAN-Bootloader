...
)
{
	...
	*rfs = fs;							/* Return pointer to the corresponding file system object */
	...
	fmt = check_fs(fs, bsect = 0);		/* Load sector 0 and check if it is an FAT-VBR (in SFD) *//*fmt=0, QJ*/
	...
	if (fmt == 1) {						/* Not an FAT-VBR, the physical drive can be partitioned */
		...
		if (tbl[4]) {						/* Is the partition existing? */
			...
			fmt = check_fs(fs, bsect);		/* Check the partition */  /*fmt=0, QJ*/
		}
	}
	...
	fasize = LD_WORD(fs->win+BPB_FATSz16);				/* Number of sectors per FAT *//*fasize=0*/
	if (!fasize) fasize = LD_DWORD(fs->win+BPB_FATSz32); /*3709*/
	fs->fsize = fasize;

	fs->n_fats = b = fs->win[BPB_NumFATs];				/* Number of FAT copies */ /*2*/
	...
	if (nclst >= MIN_FAT32) fmt = FS_FAT32;  /* fmt = 3 QJ test*/
