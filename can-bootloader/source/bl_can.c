...
extern bl_status_t g_bl_status;
...
static void bl_can_handle_msg_get_memory_map(canBASE_t *node, uint8_t *data, uint32_t len)
{
  ...
  bl_util_32tobuf_msb(&mem_map_arm[0], MEM_MAP_START_ADDR);
  bl_util_32tobuf_msb(&mem_map_arm[4], MEM_MAP_END_ADDR);
...
static void bl_can_handle_msg_write_data(canBASE_t *node, uint8_t *data, uint32_t len)
{
  ...
  if (status == CAN_CMD_SUCCESS)
  {
    if (bl_start_programming_application)
    {
      ...
      bl_checksum_calculated = bl_util_crc16(data, len);
    }
    else
    {
      bl_checksum_calculated = bl_util_crc16_incremental(bl_checksum_calculated, data, len);
    }
...
static void bl_can_handle_msg_set_start_address(canBASE_t *node, uint8_t *data, uint32_t len)
{
  ...
  do
  {
   ...
   if (len != 5)
   {
    ...
    g_bl_status.image_size += g_ulTransferSize;
  }
  while (0);
 ...
 if (bl_checksum_received == bl_checksum_calculated)
  {
    ...
    // Update BL status
    g_bl_status.app_update_status = 0x5A5A5A5A;
    g_bl_status.image_crc         = bl_checksum_received;

    Fapi_BlockProgram(g_ulUpdateStatusAddr, (uint32_t)&g_bl_status, g_ulUpdateBufferSize);
